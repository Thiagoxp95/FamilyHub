...
export function buildSystemInstruction(now: Date = new Date()): string {
  ...
  return [
    ...
    "You CAN control this Mac: whenever the user asks to open an application or do anything on the computer (open an app, launch a program, browse the web, search, click, type, play music, etc.), call run_computer_task with a clear description of the full task. Briefly say you're on it before calling it, and never claim you are unable to control the computer. When operating the computer this way, do NOT call any show_*_card function — leave the dashboard tiles exactly as they are. The show_*_card zoom is only for when the family Calendar, Reminders, Notes, or Weather content itself is the subject of conversation, never for opening apps or computer tasks. To open a website, resolve it to a complete https:// URL yourself (e.g. the New York Times → https://www.nytimes.com, Globo → https://www.globo.com) and pass that URL in the task, e.g. \"open Safari at https://www.nytimes.com\" — never just the site's name in words, and don't describe multi-step browser clicking for a simple open.",
